...
if (is_post()) {
    ...
    if ($action === 'update_do') {
        ...
        $pdo->beginTransaction();
        try {
            $pdo->prepare('UPDATE consignment_assignments SET delivery_no=?, assigned_stock=?, sale_price=?, commission_rate=?, notes=?, updated_at=NOW() WHERE id=?')->execute([$deliveryNo, $qty, $salePrice, $commissionRate, $note, $assignmentId]);
            $pdo->prepare('UPDATE consignment_inventory SET stock_balance=?, sale_price=?, commission_rate=?, updated_at=NOW() WHERE assignment_id=?')->execute([$remaining, $salePrice, $commissionRate, $assignmentId]);
            $pdo->commit();
    ...
    try {
        ...
        foreach ($mainIds as $i => $mainId) {
            ...
            $pdo->prepare('INSERT INTO consignment_assignments (consignor_id, main_inventory_id, delivery_no, assigned_stock, sale_price, commission_rate, notes, issued_by, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())')->execute([$consignorId, $mainId, $deliveryNo, $qty, $salePrice, $commissionRate, $note, auth_user()['full_name'] ?? '']);
            $assignmentId = (int)$pdo->lastInsertId();
            $pdo->prepare('INSERT INTO consignment_inventory (assignment_id, consignor_id, main_inventory_id, item_name, reference_code, item_code, stock_balance, sale_price, commission_rate, image_path, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())')->execute([$assignmentId, $consignorId, $mainId, $main['item_name'], $main['reference_code'], $main['item_code'], $qty, $salePrice, $commissionRate, $main['image_path']]);
            $saved++;
        }
